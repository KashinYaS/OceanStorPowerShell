Function Get-OceanStorController {
  ...
  PARAM (
	...
	[PARAMETER(Mandatory=$True, Position=6,HelpMessage = "Controller ID",ParameterSetName='ID')][int[]]$ID = $null
  )
  $RetVal = $null
  ...
  if ($logonsession -and ($logonsession.error.code -eq 0)) {
    ...
    switch ( $PSCmdlet.ParameterSetName )
    {
      'ID' {
	    if ( $ID.Count -eq 1 ) {
	      $URI = $RESTURI  + "Controller/" + $ID
	    }
	    else {
          $URI = $RESTURI  + "Controller"
	    }
	  }
	  default { 
	    $URI = $RESTURI  + "Controller" 
    ...
    if ($result -and ($result.error.code -eq 0)) {
			 ...
			 if ($RetVal) {
			   if ($RetVal.Count -lt $ID.Count) {
			     if (-not $Silent) {
		           ...
		           write-host "WARNING (Get-OceanStorController): Controller(s) with ID(s) $($NotFoundEncIDs) not found" -foreground "Yellow"
		         }
			   }
			 }
			 else {
		       if (-not $Silent) {
		         write-host "ERROR (Get-OceanStorController): Controller(s) with ID(s) $($ID -join ',') not found" -foreground "Red"
		       }
             }
	       }		  
		}		  
      ...
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorController): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
  ...
  foreach ($CurrentVal in $RawRetVal) {
	switch ($CurrentVal.Model) {
      '4127'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range PALM control board' }
      '4128'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range _SAS control board' }
      '4129'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C SAS entry-level control board (Hi1620S)' }
      '4132'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 4U4C high-end control board' }
      '4135'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range control Board' }
      '4136'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range SAS1711 control board' }
      '4137'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C SAS 1711 entry-level control board (Hi1620S)' }
      '4140'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 4U4C high-end 1711 control board' }
      '4141'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range SAS 1711 control board (100GE extension board)' }
      '4142'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R1C00 2U2C mid-range SAS control board (100GE extension board)' }
      '4144'   { $CurrentVal | Add-Member -NotePropertyName 'ModelNameEx' -NotePropertyValue 'V6R3C00 2U2C low-end NVMe control board' }
    }
