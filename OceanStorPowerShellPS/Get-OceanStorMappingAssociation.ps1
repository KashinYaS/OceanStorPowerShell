Function Get-OceanStorMappingAssociation {
  ...
  if ($logonsession -and ($logonsession.error.code -eq 0)) {
    ...
    {
      'HostID' {
	    $URI = $RESTURI  + "mapping/associate?ASSOCIATEOBJTYPE=21&ASSOCIATEOBJID=$($HostID)"
	  }
      'LunGroupId' {
    ...
    if ($result -and ($result.error.code -eq 0)) {
      ...
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorMappingAssociation): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
