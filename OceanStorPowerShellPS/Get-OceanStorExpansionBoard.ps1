Function Get-OceanStorExpansionBoard {
  ...
  PARAM (
	...
	[PARAMETER(Mandatory=$True, Position=6,HelpMessage = "ExpansionBoard ID",ParameterSetName='ID')][int[]]$ID = $null
  )
  $RetVal = $null
  ...
  if ($logonsession -and ($logonsession.error.code -eq 0)) {
    ...
    switch ( $PSCmdlet.ParameterSetName )
    {
      'ID' {
	    if ( $ID.Count -eq 1 ) {
	      $URI = $RESTURI  + "expboard/" + $ID
	    }
	    else {
          $URI = $RESTURI  + "expboard"
	    }
	  }
	  default { 
	    $URI = $RESTURI  + "expboard" 
    ...
    if ($result -and ($result.error.code -eq 0)) {
			 ...
			 if ($RetVal) {
			   if ($RetVal.Count -lt $ID.Count) {
			     if (-not $Silent) {
		           ...
		           write-host "WARNING (Get-OceanStorExpansionBoard): ExpansionBoard(s) with ID(s) $($NotFoundEncIDs) not found" -foreground "Yellow"
		         }
			   }
			 }
			 else {
		       if (-not $Silent) {
		         write-host "ERROR (Get-OceanStorExpansionBoard): ExpansionBoard(s) with ID(s) $($ID -join ',') not found" -foreground "Red"
		       }
             }
	       }		  
		}		  
      ...
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorExpansionBoard): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
