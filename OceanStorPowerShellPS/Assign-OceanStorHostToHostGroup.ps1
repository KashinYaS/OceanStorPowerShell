	...
	foreach ($CurrentName in $Name) {
	  ...
	  if (-not ( $CurrentHost )) {
		...
		if (-not ( $CurrentHostGroup )) {
		  ...
		  if ($WhatIf) {
			write-host "WhatIf (Assign-OceanStorHostToHostGroup): Associate Host ($($CurrentName), ID $($CurrentHost.ID)) and Host Group ($($CurrentName), ID $($CurrentHostGroup.ID))" -foreground "Green"
		  }
		  else {
			...
			if ($result -and ($result.error.code -eq 0)) {
			  ...
			  if (-not $Silent) {
				write-host "ERROR (Assign-OceanStorHostToHostGroup): Failed to associate Host ($($CurrentName), ID $($CurrentHost.ID)) and Host Group ($($CurrentName), ID $($CurrentHostGroup.ID)): $($result.error.description)" -foreground "Red"
			  }
			}
		  }
