    ...
    if ($result -and ($result.error.code -eq 0)) {
      if (-not $Name) {
		...
		if ((-not $RetVal) -and (-not $Silent)) {
		  write-host "ERROR (Get-OceanStorHostGroup): Host Group $($Name) not found" -foreground "Red"
		}
	  }
    }
    else {
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorHostGroup): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
