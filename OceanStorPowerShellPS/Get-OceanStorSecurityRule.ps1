Function Get-OceanStorSecurityRule {
  ...
  PARAM (
	...
	[PARAMETER(Mandatory=$True, Position=6,HelpMessage = "Security Rule ID",ParameterSetName='ID')][int[]]$ID = $null
  )
  $RetVal = $null
  ...
  if ($logonsession -and ($logonsession.error.code -eq 0)) {
    ...
    if ($result -and ($result.error.code -eq 0)) {
			 ...
			 if ($RetVal) {
			   if ($RetVal.Count -lt $ID.Count) {
			     if (-not $Silent) {
		           ...
		           write-host "WARNING (Get-OceanStorSecurityRule): Security Rule(s) with ID(s) $($NotFoundEncIDs) not found" -foreground "Yellow"
		         }
			   }
			 }
			 else {
		       if (-not $Silent) {
		         write-host "ERROR (Get-OceanStorSecurityRule): Security Rule(s) with ID(s) $($ID -join ',') not found" -foreground "Red"
		       }
             }
	       }		  
		}		  
      ...
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorSecurityRule): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
