Function Get-OceanStorBBU {
  ...
  PARAM (
	...
	[PARAMETER(Mandatory=$True, Position=6,HelpMessage = "BBU ID",ParameterSetName='ID')][int[]]$ID = $null
  )
  $RetVal = $null
  ...
  if ($logonsession -and ($logonsession.error.code -eq 0)) {
    ...
    switch ( $PSCmdlet.ParameterSetName )
    {
      'ID' {
	    if ( $ID.Count -eq 1 ) {
	      $URI = $RESTURI  + "backup_power/" + $ID
	    }
	    else {
          $URI = $RESTURI  + "backup_power"
	    }
	  }
	  default { 
	    $URI = $RESTURI  + "backup_power" 
    ...
    if ($result -and ($result.error.code -eq 0)) {
			 ...
			 if ($RetVal) {
			   if ($RetVal.Count -lt $ID.Count) {
			     if (-not $Silent) {
		           ...
		           write-host "WARNING (Get-OceanStorBBU): BBU(s) with ID(s) $($NotFoundEncIDs) not found" -foreground "Yellow"
		         }
			   }
			 }
			 else {
		       if (-not $Silent) {
		         write-host "ERROR (Get-OceanStorBBU): BBU(s) with ID(s) $($ID -join ',') not found" -foreground "Red"
		       }
             }
	       }		  
		}		  
      ...
      $RetVal = $null
	  if (-not $Silent) {
	    write-host "ERROR (Get-OceanStorBBU): $($result.error.code); $($result.error.description)" -foreground "Red"
	  }
    }
  }
